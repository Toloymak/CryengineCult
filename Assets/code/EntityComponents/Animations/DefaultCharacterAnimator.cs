...
namespace CryEngine.Game
{
	...
	public class DefaultCharacterAnimator : EntityComponent
	{
		[SerializeValue]
		private string _characterGeometry = "objects/characters/samplecharacter/thirdperson.cdf]";
		[SerializeValue]
		private int _characterSlot = 0;
		[SerializeValue]
		private string _controllerDefinition = "animations/Mannequin/ADB/FirstPersonControllerDefinition.xml";
		[SerializeValue]
		private string _mannequinContext = "FirstPersonCharacter";
		[SerializeValue]
		private string _animationDatabase = "animations/Mannequin/ADB/FirstPerson.adb";
		[SerializeValue]
		...
		public string StartFragmentName { get; set; } = "Idle";
		...
		private void Initialize(bool silent)
		{
			   ...
			   string.IsNullOrWhiteSpace(startFragmentName))
			{
				if(!silent)
				{
					Log.Error<DefaultCharacterAnimator>("Unable to initialize animations for Entity {0} because some of the properties are not set!", entity.Name);
				}
				return;
			}
			...
			if(controllerDefinition == null)
			{
				if(!silent)
				{
					Log.Error<DefaultCharacterAnimator>("Failed to load controller definition at {0} for Entity {1}.", mannequinControllerDefinition, entity.Name);
				}
				return;
			}
			...
			if(animationDatabase == null)
			{
				if(!silent)
				{
					Log.Error<DefaultCharacterAnimator>("Failed to load animation database at {0} for Entity {1}!", animationDatabasePath, entity.Name);
				}
				return;
			}
			...
			if(_actionController == null)
			{
				if(!silent)
				{
					Log.Error<DefaultCharacterAnimator>("Unable to create ActionController from context for Entity {0}!", entity.Name);
				}
				return;
			}
			...
			if(idleFragmentId < 0)
			{
				if(!silent)
				{
					Log.Error<DefaultCharacterAnimator>("Unable to find fragment {1} for Entity {0}!", entity.Name, startFragmentName);
				}
				return;
			}
			...
			if(skeleton == null)
			{
				if(!silent)
				{
					Log.Error<DefaultCharacterAnimator>("Unable to retrieve Animation Skeleton from Entity {0} at character-slot {1}!", entity.Name, CharacterSlot);
				}
				return;
			}
		...
		private void ActivateMannequinContext(string contextName, Character character, ControllerDefinition controllerDefinition, AnimationDatabase animationDatabase)
		{
			...
			if(scopeContextId < 0)
			{
				Log.Error<DefaultCharacterAnimator>("Failed to find {0} scope context id in controller definition for Entity {1}.", contextName, Entity.Name);
				return;
			}
		...
		public AnimationTag FindTag(string name)
		{
			...
			{
				Log.Error<DefaultCharacterAnimator>("Unable to find the specified tag because the Animator is not yet initialized properly for Entity {1}!", name, Entity.Name);
				return null;
			}
		...
		public void SetTagValue(AnimationTag tag, bool value)
		{
			...
			if(id < 0)
			{
				Log.Error<DefaultCharacterAnimator>("Tried to set the value for an invalid AnimationTag on Entity {0}!", Entity.Name);
				return;
			}
		...
		public void SetMotionParameter(MotionParameterId id, float value)
		{
			...
			if(_character == null)
			{
				Log.Error<DefaultCharacterAnimator>("Unable to retrieve Character from Entity {0} at slot {1}!", Entity.Name, CharacterSlot);
				return;
			}
